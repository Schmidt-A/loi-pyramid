...
class BaseView(object):
    ...
    def get_one(self, model):
        try:
            ...
            result = result_query.filter(getattr(model, primary_key) == path_value).one()
            ...
            owned = False
            if foreign_key:
                if getattr(filter_result, foreign_key.parent.name, None) == self.request.account.username: 
                    owned = True
            ...
            log.warning(
                'get: {} \'{}\' not found'.format(model.__tablename__, path_value))
            raise HTTPNotFound
    ...
    def get_all(self, model):
        try:
            ...
            log.warning(
                'get_all: {} queried by limit {} and offset {}'.format(model.__tablename__, limit, offset))
            ...
            get_all_data = {'total': total, 'offset': offset, model.__tablename__: []}
            ...
            log.error('get_all: could not retrieve any {}').format(model.__tablename__)
    ...
    def get_all_by_one(self, model_by, model_get):
        try:
            ...
            #TODO: should probably use effective principles or authenticated userid
            try: 
                filter_query = self.request.dbsession.query(model_by)
                filter_result = filter_query.filter(getattr(model_by, by_primary_key) == by_path_value).one()
            except NoResultFound:
                log.warning(
                'get_all_by_one: {} \'{}\' not found'.format(model_by.__tablename__, by_path_value))
                raise HTTPNotFound

            results_query = self.request.dbsession.query(model_get)
            results_query = results_query.filter(getattr(model_get, get_foreign_key.parent.name) == by_path_value)
            results_query = results_query.limit(limit).offset(offset)
            results = results_query.all()

            total_query = self.request.dbsession.query(model_get)
            total = total_query.filter(getattr(model_get, get_foreign_key.parent.name) == by_path_value).count()

            log.warning(
                'get_all_by_one: {} of {}: {} by limit {} and offset {}'.format(
            ...
            if by_foreign_key:
                ...
                if getattr(filter_result, by_foreign_key.parent.name, None) == self.request.account.username: 
                    owned = True
            #this is still too hacky
            ...
            if not (owned or self.request.account.is_admin()) and not hasattr(model_get, 'public_payload'):
                log.warning('get_all_by_one: {} by {} is not allowed to be accesed by account {}'.format(
                    model_get.__tablename__, by_path_value, self.request.account.username))
                raise HTTPForbidden

            get_all_data = {'total': total, 'offset': offset, model_get.__tablename__: []}
            for result in results:

                if get_foreign_key:
                    if getattr(result, get_foreign_key.parent.name, None) == self.request.account.username:
                        owned = True
            ...
            log.warning('get_all_by_one: returned {} from {}'.format(get_all_data, by_path_value))
            response = Response(json=get_all_data, content_type='application/json')

        except NoResultFound:
            log.warning(
                'get_all_by_one: no {} found with {} {}'.format(
                    model_get.__tablename__, model_by.__tablename__, by_path_value))
            get_all_data = []
            response = Response(json=get_all_data, content_type='application/json')
    ...
    def get_one_by_one(self, model_by, model_get):
        try:
            ...
            results_query = self.request.dbsession.query(model_get)
            result = results_query.filter(getattr(model_get, get_primary_key) == get_path_value).one()

            log.warning(
                'get_one_by_one: {}: {} of {}: {}'.format(
                    model_get.__tablename__, get_path_value, model_by.__tablename__, by_path_value))

            if not get_foreign_key:
                log.error('get_one_by_one: {}: {} of {}: {} are not linked by foreign key')
                raise HTTPInternalServerError

            if getattr(result, get_foreign_key.parent.name, None) != getattr(filter_result, by_primary_key):
                log.warning(
            ...
            owned = False
            if getattr(filter_result, get_foreign_key.parent.name, None) == self.request.account.username:
                owned = True
            elif by_foreign_key:
                if getattr(filter_result, by_foreign_key.parent.name, None) == self.request.account.username: 
                    owned = True
                ...
                if hasattr(result, 'public_payload'):
                    ...
                    log.warning(
                        'get_one_by_one: {} {} is not allowed to be accesed by account {}'.format(
                            model_get.__tablename__, get_path_value, self.request.account.username))
                    raise HTTPForbidden

            log.warning('get_one_by_one: returned {} from {}'.format(get_data, by_path_value))
            response = Response(json=get_data, content_type='application/json')
